...
#             mail = self.env['mail.template'].browse(refuse_template_id)
            ...
            if purchase_order_id.state == 'department_approval':
                ctx.update({
                    ...
                    'manager_name': _('Department Manager: ') + purchase_order_id.dept_manager_id.name,
                    'reason': rec.note,
                    })
            if purchase_order_id.state == 'assembly_approval':
                ctx.update({
                    'name': purchase_order_id.create_uid.partner_id.name,
                    'email_to': purchase_order_id.create_uid.partner_id.email,
                    'subject': _('Purchase Order: ') + purchase_order_id.name + _(' Refused'),
                    'manager_name': _('Assembly Manager: ') + purchase_order_id.assembly_manager_id.name,
                    'reason': rec.note,
                    })
            ...
            if refuse_template_id and purchase_order_id.state in ['to approve', 'assembly_approval', 'director_approval']:
