...
""".format(**dict(
            ...
            segment='-'*w,
            border='-'*(w*3 + 2),
            dur=(self.duration.encode('utf-8')).ljust(w-4),
            es=(self.es.encode('utf-8')).ljust(w-3),
            ef=(self.ef.encode('utf-8')).ljust(w-3),
            name=(self.name.encode('utf-8')).center(w),
            ls=(self.ls.encode('utf-8')).ljust(w-3),
            lf=(self.lf.encode('utf-8')).ljust(w-3),
            drag=(self.drag.encode('utf-8')).ljust(w-5),
        ))
